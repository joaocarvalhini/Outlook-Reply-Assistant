...
flowchart TB
    subgraph G["GRÁTIS — sem chamadas ao modelo"]
        T1["<b>test_assistente.py</b><br/>252 testes unitários<br/><i>o código faz o que diz?</i><br/>~1,7 s"]
    ...
    subgraph P["PAGO — chamadas reais ao modelo"]
        T5["<b>eval.py</b><br/>96 casos etiquetados<br/><i>o modelo decide bem?</i>"]
...
**Implemented** — 252 testes, `unittest` da biblioteca padrão, **zero dependências de teste**.
...
96 casos etiquetados, com métricas assimétricas. Tem documento próprio:
...
flowchart TD
    ...
    D -->|passa| E["eval.py completo<br/><i>96 casos, uma vez no fim</i>"]
    D -->|falha| A
...
> Correr os 96 casos a cada iteração é desperdício. O `eval/subset.json` tem 23 casos
