# Entregas

Fonte: /pages/politica-de-envio, /pages/perguntas-faq
Última verificação: 14 de agosto de 2026

## Prazos

- Em Portugal, o prazo de entrega é de 24 a 48 horas úteis **contadas a partir
  da expedição da encomenda**, e não a partir do momento da compra. Esta
  distinção é importante e deve ser dita ao cliente sempre que ele questionar um
  atraso.
- A encomenda é expedida depois de o pagamento ser validado, no primeiro dia
  útil seguinte à compra.
- Para os restantes países, o prazo médio de entrega é de 5 a 7 dias úteis.
- Os prazos podem variar consoante o país de destino.
- As capas personalizadas seguem o mesmo prazo de entrega dos restantes
  produtos; não têm produção extra. (Confirmado diretamente pela loja, 12 de
  agosto de 2026.)

## Custo de envio

- O envio não é gratuito em todos os casos: o custo depende do destino, do
  peso/valor da encomenda e do método de envio escolhido.
- O valor exato é calculado e apresentado no checkout, antes da confirmação da
  compra.

## Destinos

- A tripat3s envia para Portugal.
- A tripat3s envia também para Espanha, França, Áustria, Bélgica, Suíça, Alemanha,
  Itália, Países Baixos, Polónia, Suécia, Noruega, Dinamarca e Luxemburgo.

## Transportadoras

- O cliente pode escolher entre CTT Expresso e ponto de recolha DPD Pick Up.
- A escolha da transportadora é feita durante a compra.

## Processamento e seguimento

- A encomenda é processada depois de o pagamento ser validado.
- O cliente recebe as informações de seguimento por email, para acompanhar o estado
  da entrega.

## Entrega falhada

- Se o cliente não estiver em casa, a transportadora tenta nova entrega ou deixa
  instruções.
